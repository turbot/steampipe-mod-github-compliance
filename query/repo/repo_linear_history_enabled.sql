...
select
  ...
  case 
    when((b.required_linear_history_enabled is not null and (b.required_linear_history_enabled)::bool = true) or (r.allow_rebase_merge = true or r.allow_squash_merge = true)) then ' has linear history enabled.' 
    else ' has linear history disabled.' 
  end as reason,
